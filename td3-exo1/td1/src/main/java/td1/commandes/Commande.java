...
public class Commande<T> {
    ...
    // réécriture de la méthode toString en utilisant formatteurLigne, map et collect
    @Override
    public String toString() {
        return lignes.stream().map(l -> formateurLigne.apply(l)).collect(Collectors.joining());
    } 


    //définition de la fonction formatteurLigne 
    ...
    // Réécriture de la méthode cout avec map et reduce
    public Double cout(Function<Paire<Produit, Integer>, Double> calculLigne) {
        return lignes.stream().map(l -> calculLigne.apply(l)).reduce(0.0, (a,b) -> a + b) ;
    }
